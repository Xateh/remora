...
export async function processSession(sessionId: string) {
  ...
  try {
    ...
    const agentPromises = session.scopes.map(async (scope) => {
      ...
      for await (const event of stream) {
        if (event.type === 'STREAMING_URL') {
          ...
          if (event.result) {
            scopeAnalysis = typeof event.result === 'string' 
              ? event.result 
    ...
    // Step 2: Final Analysis with GPT
    console.log(`[OpenAI] Performing final gap analysis...`);
    
    const aggregatedMaterials = results.map(r => `Scope: ${r.scope}\nDiscovered Materials: ${r.materials}`).join("\n\n---\n\n");
    ...
    const analysisResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an educational assistant comparing lecture slides with top university materials to find gaps."
        },
        {
          role: "user",
    ...
    store.update(sessionId, { 
      ...
      results: {
        ...
        gapAnalysis: analysisResponse.choices[0].message.content || "No analysis generated."
      }
    });
